...
const PropertyGallery = lazy(
  () => import("../../../../../component/detailsComponants/PropertyGallery")
);
const PropertyHotspots = lazy(
  () => import("../../../../../component/detailsComponants/PropertyHotspots")
);
const SimilarProperties = lazy(
  () => import("../../../../../component/detailsComponants/SimilarProperties")
);
const HomeLoanOffers = lazy(
  () => import("../../../../../component/detailsComponants/HomeLoanOffers")
);
const PropertyServices = lazy(
  () => import("../../../../../component/detailsComponants/PropertyServices")
);
const OtherProperties = lazy(
  () => import("../../../../../component/detailsComponants/OtherProperties")
);
const VerifiedProperties = lazy(
  () => import("../../../../../component/detailsComponants/VerifiedProperties")
);
const PropertyTools = lazy(
  () => import("../../../../../component/detailsComponants/PropertyTools")
);
const InquiryModal = lazy(
  () => import("../../../../../component/detailsComponants/InquiryModal")
);
...
async function getSimilarProducts(
  ...
  try {
    ...
    const res = await fetch(
      `/api/properties/similar/${id}?${params.toString()}`,
      {
        cache: "no-store",
      }
    );
...
async function getNearbyProperties(
  ...
  const res = await fetch(
    `/api/properties?location=${location}&exclude_id=${excludeId}`,
    {
      cache: "no-store",
    }
  );
...
async function getProjectProperties(
  ...
  const res = await fetch(
    `/api/properties?project=${project}&exclude_id=${excludeId}`,
    {
      cache: "no-store",
    }
  );
...
async function getVerifiedProperties(
  ...
  const res = await fetch(
    `/api/properties?project=${project}&exclude_id=${excludeId}&verified=true`,
    {
      cache: "no-store",
    }
  );
...
export default function DetailPage() {
  ...
  const addToCompare = (propertyToAdd: Property) => {
    ...
    const isAlreadyInCompare = currentCompareList.some(
      (p) => p.id === propertyToAdd.id
    );
  ...
  return (
    <div className="bg-[#f8f4f0]  py-10">
      ...
      <div className="w-full max-w-[92%] mx-auto mt-10 bg-white rounded-xl p-6 md:p-10 shadow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 text-gray-800">
          <div>
            ...
            {formatPrice(property.price_breakup)} | ₹{formatPrice(property.emi)}
          </div>
          <div>
            <strong>Booking Amount:</strong>&nbsp;₹
            {formatPrice(property.booking_amount)}
          </div>
          <div>
        ...
        <div className="mt-6 bg-yellow-100 border border-yellow-400 text-yellow-900 px-4 py-2 rounded-md flex items-center justify-between">
          ...
          <span>
            Save upto 40% on your Dream Home Interiors from Top Brands →
          </span>
        </div>
      ...
      <Suspense fallback={<LoadingSection />}>
        <HomeLoanOffers banks={bank} onClaim={handleClaim} />
      </Suspense>
      ...
      <div className="w-full max-w-[92%] mx-auto mt-10 bg-white rounded-xl p-6 md:p-10 ">
        ...
        <div className="mt-8 pt-4 border-t border-gray-200">
          <p className="text-xs text-gray-500">
            Disclaimer: {process.env.NEXT_PUBLIC_NAME} has endeavoured to
            ascertain the requirement of RERA registration. However, the
            advertiser claims that there is no requirement for such
